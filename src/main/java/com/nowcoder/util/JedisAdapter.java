...
public class JedisAdapter implements InitializingBean {
    ...
    public static void main(String[] args) {
        ...
        //get set
        jedis.set("hello", "world");//比如用于帖子浏览数
        print(1, jedis.get("hello"));
        jedis.rename("hello", "newhello");
        print(1,jedis.get("newhello"));
        jedis.setex("hello2", 15, "world");//比如用于带过期时间的验证码
        ...
        jedis.del(listName);
        for(int i = 0; i < 10; ++i){
            jedis.lpush(listName, "a" + String.valueOf(i));//比如保存最近的关注列表（比如最近十个人的关注列表）
        }
        ...
        print(15, jedis.hexists(userKey, "email"));//用于判断一个对象是否有某一属性
        print(16, jedis.hexists(userKey, "age"));
        print(16, jedis.hkeys(userKey));
        print(16, jedis.hvals(userKey));
        jedis.hsetnx(userKey, "school", "DUT");//如果不存在某一属性，则写入该属性！
        jedis.hsetnx(userKey, "name", "zjs");
        ...
        print(40, jedis.zlexcount(setKey, "-", "+"));//lexical(词法)--->zlexcount
        ...
        JedisPool pool = new JedisPool("redis://localhost:6379/9");//连接池默认有8条线程可用，所以用完一定要close()
        /*for(int i = 0; i < 100; ++i){
